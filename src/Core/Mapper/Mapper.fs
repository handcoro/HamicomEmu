...
module Mapper =
  ...
  let readNrom addr cart = // PRG ROM の読み込み
    ...
    let addr2 = if cart.prgRom.Length = 0x4000 && addr' >= 0x4000 then addr' % 0x4000 else addr' // 16KB ROM の場合はミラーリング
    cart.prgRom[addr2]

  let getOffset calcBank bankSize baseAddr =
    calcBank * bankSize - baseAddr
  ...
  let writePrgVRC1 addr value state =
    ...
    // ミラーリングと CHR バンク 0, 1 の上位ビット
      ...
      { state with mirroring = mirror; chrSelect0 = chrSelect0; chrSelect1 = chrSelect1 }
  ...
  let ppuReadRange startAddr endAddr cart mapper =
    ...
    | VRC1 _
    | J87 _ ->
      Array.init (endAddr - startAddr + 1) (fun i -> ppuRead (startAddr + i) cart mapper)
