...
module Factory =
    ...
    /// ID から Mapper を生成
    let create n (prg : byte[]) (chr : byte[]) hardwiredMirroringOpt =
        let hwm = Option.defaultValue Vertical hardwiredMirroringOpt
        match n with
        | 0 -> NROM ()
        | 1 -> MMC1 Mmc1.init
        | 2 -> UxROM { bankSelect = 0uy }
        | 3 -> CNROM { bankSelect = 0uy }
        | 4 -> MMC3 (Mmc3.init prg chr hwm)
        | 19 -> Namco163 Namco163.init
        | 66 -> GxROM Gxrom.init
        | 75 -> VRC1 (Vrc1.init hwm)
