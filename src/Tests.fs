  ...
  testList "All Tests" [
    ...
    testList "PPU Tests" [
      ...
      test "writeToAddressRegister (second) completes t, copies t to v, sets w=false" {
        let ppu = Ppu.initial (testRom [||]) |> Ppu.writeToAddressRegister 0x21uy
      ...
      test "Status read resets w" {
        let ppu = Ppu.initial (testRom [||]) |> Ppu.writeToScrollRegister 0x12uy
